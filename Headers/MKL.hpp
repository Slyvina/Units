	...
	namespace Units {
		...
		public:
			...
			inline std::string uVersion() {
				char ret[10];
				sprintf_s(ret, "%02d.%02d.%02d", Day, Month, SYear());
				return ret;
			}
		...
		inline void MKL_VersionP(std::string File, std::string DTE) {
			...
			std::string DTEA[3]{ "","","" };
			{
				...
				if (months.count(DTEA[0])) MKL_Version(File,std::stoi(DTEA[2]), months[DTEA[0]], std::stoi(DTEA[1]));
				else if (months.count(DTEA[1])) MKL_Version(File, std::stoi(DTEA[2]), months[DTEA[1]], std::stoi(DTEA[0]));
